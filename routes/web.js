...
const { queriesMysql } = require("./queries.js");
const {
  addPriceColor,
  getFileExtension,
  getAllAPIKeys,
...
router.get("/update_queries", async (req, res) => {
  try {
    ...
    if (newQueries && JSON.parse(newQueries)?.length > 0) {
      ...
      res.send("There is no query found");
    }
  } catch (err) {
...
router.get("/get_plans", async (req, res) => {
  try {
    ...
    const newData = addPriceColor(data, "price");
...
router.post("/update_api_keys", adminValidator, async (req, res) => {
  try {
    const { open_ai, gemini_ai } = req.body;
    await query(`UPDATE api_keys SET open_ai = ?, gemini_ai = ?`, [
      open_ai,
      gemini_ai,
    ]);
    res.json({
...
// get keys
router.get("/get_keys", async (req, res) => {
  try {
